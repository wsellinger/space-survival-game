...
/// <summary>
/// Draws the health/O2 bars in screen space (no camera transform), bottom-center of the
...
/// resource's remaining percentage (tinted the bar's color) and centered so it shrinks
/// symmetrically from both ends toward the middle rather than just from one side, and an
/// outline on top (always full bar length) so the frame reads as constant while only the fill
