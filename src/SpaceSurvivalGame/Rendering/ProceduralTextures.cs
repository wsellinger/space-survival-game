...
public static class ProceduralTextures
{
    ...
    /// so drawing it with rotation = the body's angle needs no extra offset.
    /// </summary>
    public static Texture2D CreateRightFacingTriangle(GraphicsDevice graphicsDevice, int size, Color color)
    {
        ...
        for (var y = 0; y < size; y++)
        {
            for (var x = 0; x < size; x++)
            {
                ...
                data[y * size + x] = IsInsideTriangle(point, tip, tailTop, tailBottom) ? color : Color.Transparent;
            }
        }
