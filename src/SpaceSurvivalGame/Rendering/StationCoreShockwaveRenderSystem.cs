...
/// nothing out to ShockwaveRadiusMeters over ShockwaveDurationSeconds while fading out, tracked
...
/// ShockwaveRadiusMeters*2 (in pixels) so scale 1 is the ring's true final size — the ring only
...
public static class StationCoreShockwaveRenderSystem
{
    ...
    {
        var color = ColorHex.Parse(config.ShockwaveColorHex);
        ...
        {
            if (core.ShockwaveElapsedSeconds < 0f || core.ShockwaveElapsedSeconds >= config.ShockwaveDurationSeconds) return;

            var progress = core.ShockwaveElapsedSeconds / config.ShockwaveDurationSeconds;
            var alphaFraction = (1f - progress) * config.ShockwaveMaxAlpha;
