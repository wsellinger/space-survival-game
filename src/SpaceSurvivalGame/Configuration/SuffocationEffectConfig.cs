...
public class PixelationConfig
{
    public bool Enabled { get; set; } = false;
    public float MaxBlockSizePixels { get; set; } = 24f;
}
...
public class NoiseConfig
{
    ...
    // give chunkier grain.
    public float GrainSizePixels { get; set; } = 1f;

    // Noise blend strength at full effect intensity; 0 disables it entirely.
    public float MaxIntensity { get; set; } = 0.35f;
...
public class GrayscaleConfig
{
    ...
    public float EaseExponent { get; set; } = 0.4f;
}
...
public class VignetteConfig
{
    ...
    // (fully black) by SuffocationEffectConfig.EffectDurationSeconds.
    public float StartRadius { get; set; } = 1.5f;

    // Softness of the vignette's edge, in the same normalized units.
    public float FeatherRadius { get; set; } = 0.2f;
    ...
    public float EaseExponent { get; set; } = 2.5f;
}
...
public class SuffocationEffectConfig
{
    ...
    public PixelationConfig Pixelation { get; set; } = new();
